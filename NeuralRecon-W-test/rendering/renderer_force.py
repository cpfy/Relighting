...
class NeuconWRenderer:
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s, step):
        ...
        print(f"[Out]sdf device = {sdf.device}")
        print(f"[Out]z_vals device = {z_vals.device}")
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, new_z_vals, sdf, last=False):
        ...
        if not last:
            ...
            new_sdf = torch.ones(_n_rays, _n_samples)
            if self.use_gpu:
                new_sdf = new_sdf.cuda()
    ...
    def sparse_sampler(self, rays_o, rays_d, near, far, perturb):
        ...
        if self.n_importance > 0:
            with torch.no_grad():  # 该语句 wrap 起来的部分将不会计算梯度，也不会进行反向传播
                ...
                # sdf不重要，不妨直接扔了！（先还原，另一个renderer_cpu中处理）
    ...
    def render(
        ...
        if not self.origin.is_cuda and self.use_gpu:
